...
__ALIGN_BEGIN static uint8_t CUSTOM_HID_ReportDesc_FS[USBD_CUSTOM_HID_REPORT_DESC_SIZE] __ALIGN_END =
{
    ...
    0x95, 0x40,                    //   REPORT_COUNT (64)  ??REPORT_COUNT
...
{
  /* USER CODE BEGIN 6 */
	  USBD_CUSTOM_HID_HandleTypeDef   *hhid; //??????USBD_CUSTOM_HID_HandleTypeDef??????
  hhid = (USBD_CUSTOM_HID_HandleTypeDef*)hUsbDeviceFS.pClassData;//??USB?????????
    
    for(int i=0;i<64;i++) 
    {
        USB_Recive_Buffer[i]=hhid->Report_buf[i];  //??????????????????(Report_buf[i]?USB??????)
