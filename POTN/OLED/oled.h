...
#define OLED_IO 1  //定义OLED IO口的连接方式 ， 如果是 0 则是 最小系统板子的连接方式，如果是1 则是 3D打印外壳后的连接方式
	...
	#define OLED_SCLK_Clr() HAL_GPIO_WritePin(GPIOB,GPIO_PIN_7,GPIO_PIN_RESET)  //时钟 D0
	#define OLED_SCLK_Set() HAL_GPIO_WritePin(GPIOB,GPIO_PIN_7,GPIO_PIN_SET)

	#define OLED_SDIN_Clr() HAL_GPIO_WritePin(GPIOB,GPIO_PIN_8,GPIO_PIN_RESET)//DIN  数据 D1
	...
	#define OLED_DC_Clr() HAL_GPIO_WritePin(GPIOC,GPIO_PIN_13,GPIO_PIN_RESET)//DC
